...
def get_data(start_year, end_year, conf_label=False):
    ...
    for game in tourney_data_subset.iterrows():
        ...
        if not conf_label: # if we do not want the conference identifier on the seed
        ...
        if random.random() < 0.5: # randomly choose to order the data as winner, loser, 1 (label) or loser, winner, 0 (label)
            game_data.append(w_seed)
            game_data.append(l_seed)
            game_data.append(1)
        else:
            game_data.append(l_seed)
            game_data.append(w_seed)
